...
namespace MalieTools
{
	namespace CamelliaDecryption
	{
		...
		static const uint32_t g_SBOX_TABLE1[] = 
		{
		...
		static const uint32_t g_SBOX_TABLE2[] =
		{
		...
		static const uint32_t g_SBOX_TABLE3[] =
		{
		...
		static const uint32_t g_SBOX_TABLE4[] =
		{
		...
		uint32_t MutateValue(uint32_t Value) { return (_rotl(Value, 8) & 0x00FF00FF) | (_rotr(Value, 8) & 0xFF00FF00); }
		...
		void DecryptBlock(const uint32_t nOffset, const uint32_t* pKeyTable, uint8_t* pBlock)
		{
			...
			uint32_t* src_block = (uint32_t*)pBlock;

			const uint32_t* key = pKeyTable;
			const uint32_t  iterations = 3;

			uint32_t roll_bits = ((nOffset >> 4) & 0x0F) + 16;
